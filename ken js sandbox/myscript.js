...
function  myStockFunction () {
    fetch('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=BTC&apikey=PZRHDEAG8FRM7HAA')
        .then(response => response.json())
        .then(data => {
            // Extract the stock price from the API response
            const stockPrice = data['Global Quote']['05. price'];
            // Display the stock price
            document.getElementById('stockPrice').textContent = 'BTC Stock Price: ' + stockPrice;
        })
        .catch(error => {
